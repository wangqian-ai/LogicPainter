...
def paint_over_canvas(canvas: torch.Tensor, stroke: torch.Tensor):
  darkness_mask = torch.mean(stroke, dim=1, keepdim=True)  # Take mean over color channels 1,3,288,480-->1,1,288,480
  ...
  # normalizer, _ = torch.min(darkness_mask, dim=2, keepdim=True)
  # normalizer, _ = torch.min(normalizer, dim=3, keepdim=True) # max darkness_mask
  darkness_mask = darkness_mask / normalizer
  blended = darkness_mask * stroke + (1-darkness_mask) * canvas
  # blended = stroke + (1-darkness_mask) * canvas
  return blended
...
class NeuralCanvasStitched(nn.Module):
  ...
  def forward(self, actions: torch.Tensor, next_canvas, resize=False):
    """
    actions: tensor of shape (num_strokes, batch_size, action_size)
    ...
    intermediate_canvases = []
#     intermediate_canvases.append(next_canvas.detach().cpu())
    intermediate_canvases.append(F.interpolate(next_canvas.detach().cpu(), (1024,1024)))
...
#         for local_stroke_idx in range(random.randint(1,self.strokes_per_block)):
          ...
          if resize==False:
            ...
            intermediate_canvases.append(F.interpolate(next_canvas.detach().cpu(), (1024,1024)))  # Is this efficient? Maybe we should only keep it in memory in certain cases?
        block_ctr += 1
    return next_canvas, intermediate_canvases
